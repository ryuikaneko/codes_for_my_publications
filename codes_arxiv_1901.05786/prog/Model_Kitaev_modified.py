...
def Initialize_Tensors(Tn,seed,initial_type=0,initial_direction=0,random_amp=0.01):
    ...
    elif initial_type == 11:
        for i in range(0,N_UNIT):
            ix = i%LX_ori
            iy = i/LX_ori
#            a = 0.88807383
#            b = 0.32505758
            a = np.sqrt((3.0+np.sqrt(3.0))/6.0)
            b = np.sqrt((3.0-np.sqrt(3.0))/12.0)
            if (ix + iy)%2 == 0:
                ## A sub lattice
                Tn[i][0,0,0,0,0] = a
                Tn[i][0,0,0,0,1] = b + b*1.0j
                Tn[i][0,0,2,2,0] = b + b*1.0j
                Tn[i][0,0,2,2,1] = a

                Tn[i][0,1,0,1,0] = a
                Tn[i][0,1,0,1,1] = -b - b*1.0j
                Tn[i][0,1,2,3,0] = b + b*1.0j
                Tn[i][0,1,2,3,1] = -a 

                Tn[i][0,2,1,0,0] = b - b*1.0j
                Tn[i][0,2,1,0,1] = a*1.0j
                Tn[i][0,2,3,2,0] = -a*1.0j
                Tn[i][0,2,3,2,1] = -b + b*1.0j

                Tn[i][0,3,1,1,0] = b - b*1.0j
                Tn[i][0,3,1,1,1] = -a*1.0j
                Tn[i][0,3,3,3,0] = -a*1.0j
                Tn[i][0,3,3,3,1] = b - b*1.0j
                ...
                Tn[i][0,0,0,0,1] = b + b*1.0j
                Tn[i][0,2,0,2,0] = a
                Tn[i][0,2,0,2,1] = -b - b*1.0j

                Tn[i][1,1,0,0,0] = b + b*1.0j
                Tn[i][1,1,0,0,1] = a 
                Tn[i][1,3,0,2,0] = -b - b*1.0j
                Tn[i][1,3,0,2,1] = a

                Tn[i][2,0,0,1,0] = b - b*1.0j
                Tn[i][2,0,0,1,1] = a*1.0j
                Tn[i][2,2,0,3,0] = -b + b*1.0j
                Tn[i][2,2,0,3,1] = a*1.0j

                Tn[i][3,1,0,1,0] = a*1.0j
                Tn[i][3,1,0,1,1] = b - b*1.0j
                Tn[i][3,3,0,3,0] = a*1.0j
                Tn[i][3,3,0,3,1] = -b + b*1.0j

    elif initial_type == 12:
        for i in range(0,N_UNIT):
            ix = i%LX_ori
            iy = i/LX_ori
#            a = 0.88807383
#            b = 0.32505758
            ...
            if (ix + iy)%2 == 0:
                ## A sub lattice
                Tn[i][0,0,0,0,0] = +a
                Tn[i][0,0,0,0,1] = +b +b*1j
                Tn[i][0,0,1,1,0] = +b +b*1j
                Tn[i][0,0,1,1,1] = +a
                Tn[i][0,1,0,1,0] = +a
                Tn[i][0,1,0,1,1] = -b -b*1j
                Tn[i][0,1,1,0,0] = -b +b*1j
                Tn[i][0,1,1,0,1] = -a*1j   
            else:
                ## B sub lattice
                Tn[i][0,0,0,0,0] = +a
                ...
                Tn[i][0,1,0,1,0] = +a
                Tn[i][0,1,0,1,1] = -b -b*1j
                Tn[i][1,0,0,1,0] = -b +b*1j
                Tn[i][1,0,0,1,1] = -a*1j
                Tn[i][1,1,0,0,0] = +b +b*1j
                Tn[i][1,1,0,0,1] = +a
